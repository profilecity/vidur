...
<template>
  <div
    class="col-span-full sm:col-span-6 xl:col-span-4 bg-white shadow-sm rounded-xl border border-zinc-200"
  >
    <div class="flex flex-col h-full p-5">
      <header>
        <div class="flex items-center justify-between">
          ...
          >
            <Icon
              class="w-5 h-5 shrink-0 text-zinc-800"
              name="hugeicons:permanent-job"
            />
          </div>
      ...
      <div class="grow mt-2">
        <NuxtLink
          class="inline-flex text-gray-800 hover:text-gray-900 mb-1"
      ...
      <footer class="mt-5">
        <div class="text-sm font-medium text-gray-500 mb-2">
          Created {{ timeAgo(new Date(posting.createdAt)) }}
        </div>
        <div class="flex justify-between items-center">
          ...
          <div
            class="text-xs font-medium rounded-lg text-center px-2.5 py-1 bg-sky-100 text-sky-700"
            v-else
          >
            Draft
          </div>
          <InputButton
            ...
            variant="outline"
            size="icon"
            :to="'/admin/postings/edit?id=' + posting.id"
