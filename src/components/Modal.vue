...
<template>
  <DialogRoot v-model:open="isModalOpen">
    ...
    <DialogPortal>
      ...
      >
        <DialogTitle class="font-bold text-zinc-900 font-noto" v-if="title">{{
          title
        }}</DialogTitle>
        <DialogDescription class="text-sm text-zinc-500" v-if="description">{{
          description
        }}</DialogDescription>
        <slot name="content" :close="close" />
        <DialogClose
          class="absolute top-[10px] right-[10px] inline-flex items-center justify-center"
        >
