...
type AddOrRemoveMember = z.infer<typeof addMemberSchema> | z.infer<typeof removeMemberSchema>;

const useMembersState = () => useState<User[]>('settings-members', () => []);
const useMembersFetchedState = () => useState<boolean>('settings-members-first-fetch', () => false);
...
export async function useMembers() {
  ...
  if (!membersFetched.value) {
    await membersApiCall.execute().then(() => setMembers(membersApiCall.data.value));
  ...
  const makeAddOrRemoveMember = (method: 'POST' | 'DELETE') => async (body: AddOrRemoveMember) => {
    try {
