...
<template>
  ...
  <main v-else>
    ...
    <h4 class="text-sm text-zinc-600 mt-2 mb-6 text-center">
    ...
    <section class="w-full mt-16 flex items-center">
      ...
      <div class="w-1/2 mx-auto" v-if="currentStep == 3">
        <div class="text-center">
          <div
            class="text-2xl text-zinc-700 font-noto"
            v-if="settingUpCareerSite"
          >
            Setting up the career site. <br />
            <Icon
              name="svg-spinners:blocks-shuffle-3"
              class="w-12 h-12 mt-10"
            />
          </div>
          <div v-else-if="!errorSettingUpCareerSite">
            <Icon
              ...
              class="w-32 h-32"
            />
            <h1 class="text-2xl font-noto text-zinc-800 font-bold mb-8">
              Nice to have you, {{ orgName }}. 🙌
            </h1>
            <InputButton as="a" href="/admin">Go to admin console</InputButton>
          </div>
          <div class="text-2xl text-red-600 font-noto" v-else>
            Something went wrong!<br />There was an error setting up the career
            site.
          </div>
        </div>
      </div>
