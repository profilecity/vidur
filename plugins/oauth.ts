...
export default defineNuxtPlugin(async (nuxtApp) => {
  addRouteMiddleware('login', async (to) => {
    ...
    const nextURLCookie = await nuxtApp.runWithContext(() =>
      useCookie('oauth_next_url'),
    );
    ...
    if (
      ...
      if (to.query['code']) {
        ...
        // This means leg 1 is going on
        const state = await nuxtApp.runWithContext(() =>
          useCookie<string>('oauth_state'),
        );
        ...
        params.set(
          'code_challenge',
          await getChallengeFromVerifier(codeVerifier.value),
        );
        params.set('code_challenge_method', 'S256');

        return nuxtApp.runWithContext(() =>
          navigateTo(
            `${authConfig.endpoints.authorization}?${params.toString()}`,
            { external: true },
          ),
        );
      }
  ...
  addRouteMiddleware('auth', async (to) => {
    ...
    if (!getBearerToken()) {
      const nextURL = await nuxtApp.runWithContext(() =>
        useCookie<string>('oauth_next_url'),
      );
